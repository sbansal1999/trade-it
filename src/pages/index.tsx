...
export default function Home() {
  ...
  const { data: userIDFromLocalStorage } = useQuery({
    queryKey: ["userID"],
    queryFn: fetchUserIDFromLocalStorage,
  });

  // fetching user data from DB
  // TODO: ideally should be stored in context
  const { data: userData, isError } = trpc.users.getUserByID.useQuery(
    userIDFromLocalStorage || "",
    {
      refetchOnWindowFocus: false,
      enabled: Boolean(userIDFromLocalStorage),
    }
  );

  function fetchUserIDFromLocalStorage() {
    // simulates fetching user from local storage
    const userIDFetchedFromLocalStorage =
      "8a0a7f28-f7f5-47bb-800c-bfad230fffde";

    return userIDFetchedFromLocalStorage;
  }
  ...
  const handleConfirmOrder = async () => {
    if (!selectedStock || !selectedStockQuote || !userData) {
      return;
    }
  ...
  return (
    <>
      ...
      <main>
        ...
        {JSON.stringify(userIDFromLocalStorage)}
        <br />
...
}> = ({ text, value, noDollarSign: noDollars }) => (
